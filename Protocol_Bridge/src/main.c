...
int main()
{
	...
	MM_BREAK_IF( pthread_create( &trd_version_checker , NULL , version_checker , ( pass_p )_g ) != PTHREAD_CREATE_OK , errGeneral , 0 , "Failed to create thread" );

	pthread_t trd_config_loader;
	MM_BREAK_IF( pthread_create( &trd_config_loader , NULL , config_loader , ( pass_p )_g ) != PTHREAD_CREATE_OK , errGeneral , 0 , "Failed to create thread" );

	pthread_t trd_config_executer;
	MM_BREAK_IF( pthread_create( &trd_config_executer , NULL , config_executer , ( pass_p )_g ) != PTHREAD_CREATE_OK , errGeneral , 0 , "Failed to create thread" );
	...
	MM_BREAK_IF( pthread_create( &trd_watchdog , NULL , watchdog_executer , ( pass_p )_g ) != PTHREAD_CREATE_OK , errGeneral , 0 , "Failed to create thread" );
