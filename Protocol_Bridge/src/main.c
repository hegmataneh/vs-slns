...
#ifndef section_bridge_passive_content
...
#ifndef section_bridge_active_content
...
_THREAD_FXN void * thread_udp_connection_proc( void * src_pb )
{
	...
	if ( twd.threadId == 0 )
	{
		...
		twd.cal = thread_udp_connection_proc;
	}

	//pAB pb = ( pAB )src_pb;
	//G * _g = ( pG )pb->apcfg.m.m.temp_data._g;
	////SYS_ALIVE_CHECK();
...
void apply_new_protocol_bridge_config( struct App_Data * _g , pAB pb , Bcfg * new_pcfg )
{
	INIT_BREAKABLE_FXN();
	...
	//memcpy( &pb->apcfg , new_pcfg , sizeof( Bcfg ) );
...
void stop_protocol_bridge( struct App_Data * _g , pAB pb )
{
	////INIT_BREAKABLE_FXN();
...
void apply_protocol_bridge_new_cfg_changes( struct App_Data * _g , Bcfg * prev_pcfg , Bcfg * new_pcfg )
{
	////INIT_BREAKABLE_FXN();
...
void remove_protocol_bridge( struct App_Data * _g , Bcfg * pcfg )
{
	////INIT_BREAKABLE_FXN();
...
void add_new_protocol_bridge( struct App_Data * _g , Bcfg * new_pcfg )
{
	//INIT_BREAKABLE_FXN();
	...
	//memcpy( &_g->bridges.pb_holders[ new_pcfg_placement_index ].alc_pb->apcfg , new_pcfg , sizeof( Bcfg ) );
...
_THREAD_FXN void * main_manager( void * app_data )
{
	INIT_BREAKABLE_FXN();
	//G * _g = ( pG )app_data;
...
int main()
{
	...
	pthread_t trd_main_manager;
	MM_BREAK_IF( pthread_create( &trd_main_manager , NULL , main_manager , ( void * )_g ) != PTHREAD_CREATE_OK , errGeneral , 0 , "Failed to create thread" );

	//M_BREAK_IF( pthread_join(  , NULL ) != PTHREAD_JOIN_OK , errGeneral , 0 );
