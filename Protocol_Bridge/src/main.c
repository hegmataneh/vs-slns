...
void * bottleneck_thread_proc( void * src_pb )
{
	INIT_BREAKABLE_FXN();
	...
	do
	{
		...
		while ( 1 )
		{
			...
			if ( new_udp_listen )
			{
				...
				{
					MM_BREAK_IF( pthread_create( &_g->bridges.bottleneck_thread.trd_id , NULL , bottleneck_thread_proc , pb ) != PTHREAD_CREATE_OK , errGeneral , 0 , "thread creation failed" );
