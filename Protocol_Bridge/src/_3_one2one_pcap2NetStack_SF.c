...
void quit_interrupt_dist_one2one_pcap2kernelDefaultStack_SF( pass_p src_pb , int v )
{
	AB * pb = ( AB * )src_pb;
	if ( pb->trd.t.p_one2one_pcap2NetStack_SF->handle )
	{
		pcap_breakloop( pb->trd.t.p_one2one_pcap2NetStack_SF->handle ); // in case we're inside pcap_loop
		pcap_close( pb->trd.t.p_one2one_pcap2NetStack_SF->handle );
	}
}

_PRIVATE_FXN _CALLBACK_FXN status buffer_push_one2one_pcap2kernelDefaultStack_SF( pass_p data , buffer buf , int payload_len )
{
	AB * pb = ( AB * )data;
	return vcbuf_nb_push( &pb->trd.t.p_one2one_pcap2NetStack_SF->cbuf , buf , payload_len );
}

_THREAD_FXN void_p proc_one2one_pcap2NetStack_SF_udp_pcap( pass_p src_pb )
{
	INIT_BREAKABLE_FXN();
	AB * pb = ( AB * )src_pb;
	...
	M_BREAK_STAT( distributor_subscribe( &pb->trd.base.buffer_push_distributor , SUB_DIRECT_ONE_CALL_BUFFER_INT ,
		SUB_FXN( buffer_push_one2one_pcap2kernelDefaultStack_SF ) , src_pb ) , 1 );
...
_THREAD_FXN void_p proc_one2one_pcap2NetStack_SF_tcp_out( pass_p src_pb )
{
	INIT_BREAKABLE_FXN();
	...
	shrt_path pth;
	mk_shrt_path( pb , &pth );
	pth.cbuf = &pb->trd.t.p_one2one_pcap2NetStack_SF->cbuf;
	pth.buf_pop_distr = &pb->trd.t.p_one2one_pcap2NetStack_SF->buffer_pop_distributor;
